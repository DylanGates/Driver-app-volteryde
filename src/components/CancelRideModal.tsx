...
export default function CancelRideModal({
  ...
  const commitmentFee = 1.50;
  const chargedFare = 2.00;
      ...
      <div className="relative z-10 w-full max-w-2xl rounded-2xl bg-background shadow-2xl max-h-[90vh] overflow-hidden animate-in zoom-in-95 fade-in duration-200">
        ...
        <div className="bg-red-500 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-white">Cancel Ride</h2>
          </div>
          <button
            onClick={handleClose}
          ...
          <div className="p-6 space-y-6">
                ...
                {cancellationReasons.map((reason) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`flex-shrink-0 ${selectedReason === reason.id ? "text-brand-primary" : "text-gray-400"}`}>
                        {reason.icon}
                      </div>
            ...
            <div>
              <button
                ...
                className="w-full px-4 py-3 border-2 border-foreground/20 rounded-full hover:bg-foreground/5 transition-colors flex items-center justify-center gap-2"
              >
            ...
            <div className="border-2 border-foreground/20 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-semibold text-foreground">Refund Calculation</h3>
              </div>

              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-foreground/70">Original Fare</span>
                  <span className="font-medium">GH₵{originalFare.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-foreground/70">Distance Traveled ({distanceTraveled} km)</span>
                  <span className="font-medium text-red-500">- GH₵{chargedFare.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-foreground/70">10% Commitment fee</span>
                  <span className="font-medium text-red-500">- GH₵{commitmentFee.toFixed(2)}</span>
                </div>
                <div className="pt-3 border-t border-foreground/20 flex justify-between">
                  <span className="font-semibold text-red-500">Refund Amount</span>
                  <span className="font-bold text-red-500">GH₵{refundAmount.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                onClick={handleClose}
                className="flex-1 px-6 py-3 text-base font-medium border-2 border-foreground/20 rounded-full hover:bg-foreground/5 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 px-6 py-3 text-base font-medium bg-red-500 text-white rounded-full hover:bg-red-600 transition-all shadow-sm hover:shadow disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Confirm
              </button>
            </div>
